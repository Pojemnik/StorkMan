...
Object::Object(Vectorf pos_, const std::array<const sf::Texture*, 3>* texture_, float height_,
	int flip_, float angle_, sf::Color color, float parallax_) : pos(pos_),
	height(height_), flip(flip_), sprite(*texture_->at(0)), parallax(parallax_)
{
	float scale = height * context.global_scale / texture_->at(0)->getSize().y;
...
void Object::draw(Gbuffer& target, sf::RenderStates states)
{
...
Moving_object::Moving_object(Vectorf pos_, const std::array<const sf::Texture*, 3>* texture_, float height_,
...
void Moving_object::draw(Gbuffer& target, sf::RenderStates states)
{
	auto matrix = states.transform.getMatrix();
	float px, py;
	px = matrix[12] * parallax;
...
{
	sprite.setTexture(*animation->get_texture()->at(0));
}
...
void Moving_animated_object::draw(Gbuffer& target, sf::RenderStates states)
{
	auto matrix = states.transform.getMatrix();
	float px, py;
	px = matrix[12] * parallax;
