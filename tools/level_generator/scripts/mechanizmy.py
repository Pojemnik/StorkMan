...
def taśmociąg_jednostronny(x=0, y=0, długość=10, grubość=0.5, obrót=0, prędkość=2, segmentowanie=10, bariery=1, widoczność_barier=1, tekstura=podstawy.domyślna_tekstura, elementy=(0,0), zwróć_tekst=False):
    ...
    for i in range(segmentowanie-1):
        ...
        s = s + podstawy.ruchoma_platforma(x=x+długość_x*i,y=y+długość_y*i,tekstura=tekstura,wierzchołki=v,ruch=p,zwróć_tekst=True)
    if bariery == 1:
        if widoczność_barier==1:
            s = s + podstawy.platforma(x=x+długość_x*(segmentowanie-1),y=y+długość_y*(segmentowanie-1),tekstura=tekstura,wierzchołki=v,zwróć_tekst=True)
