from level_base import level

lvl = level('Elektrownia_Weglowa_X3Y2')

import podstawy
podstawy.lvl = lvl
import operacje
import pułapki
import struktury
import mechanizmy
import obiekty
import chłodnia_kominowa
import ruchy
import elektrownia_węglowa
import grafiki

#zdarzenia
podstawy.zdarzenie("LightOn")
podstawy.zdarzenie("LightOff")
podstawy.zdarzenie("PowerPlantOn")
podstawy.zdarzenie("PowerPlantOff")
podstawy.zdarzenie("TechnicalHash1Open")
podstawy.zdarzenie("TechnicalHash1Close")
podstawy.zdarzenie("TechnicalHash2Open")
podstawy.zdarzenie("TechnicalHash2Close")
podstawy.zdarzenie("OutPipeEntered")
podstawy.zdarzenie("OutPipeLeft")
#ziemia
podstawy.platforma(x=0,y=47,wierzchołki=["v",4,0,0,0,3,6.5,3,6.5,0],tekstura="dirt,0",powierzchnia="piach")
podstawy.platforma(x=6.5,y=48,wierzchołki=["v",4,0,0,0,2,43.5,2,43.5,0],tekstura="dirt,0",powierzchnia="piach")
#budynek generatora
struktury.komnata(x=6.5,y=0,warstwa=8,kolor=[128,128,128],szerokość=43.5,wysokość=48,grubość_prawej=1,grubość_lewej=1,grubość_górnej=1,grubość_dolnej=1,tekstura="bricks,0",tekstura_tła="asphalt,0",otwory_lewe=[3,8.5,3,27.5,3,43.5,2.5],powierzchnia="posadzka_hala")
#budynek generatora - parter
generator = "<state>\n"
generator += elektrownia_węglowa.generator(x=9.5,y=47.1,warstwa=5,turbiny=[3,"TURBINA_04","TURBINA_05","TURBINA_06"],zdarzenia_dźwięku=["et",3,"default","false","PowerPlantOn","true","PowerPlantOff","false"],zwróć_tekst=True)
generator += "</state>\n<state>\n"
generator += elektrownia_węglowa.generator(x=9.5,y=47.1,warstwa=5,turbiny=[3,None,None,None],wstrząsy=[False],podstawa_wzmocnienia=1,czas_obrotu_wału=360,zwróć_tekst=True)
generator += "</state>\n"
generator += podstawy.wyzwalacz_stanu(0,"PowerPlantOn",True)
generator += podstawy.wyzwalacz_stanu(1,"PowerPlantOff",True)
generator += podstawy.wyzwalacz_stanu(1,"default",True)
podstawy.zapis(generator)
struktury.komnata(x=41.5,y=39.5,warstwa=8,kolor=[128,128,128],szerokość=7.5,wysokość=7.5,grubość_prawej=1,grubość_lewej=1,grubość_górnej=1,grubość_dolnej=1,tekstura="bricks,0",tło=False,otwory_lewe=[1,3.5,2],powierzchnia="posadzka_hala")
struktury.schody(x=41.5,y=47,obrót=-1,wysokość=1,długość=1.5,stopnie=3,przesunięcie=0.5,kolor=[128,128,128],powierzchnia="posadzka_hala")
struktury.element_podłużny(x=14.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=18.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=23.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=27.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=32.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=36.5,y=40.5,warstwa=4,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=8,y=40.5,warstwa=7,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[3,0,0,0,1.5,-1,2.5])
struktury.element_podłużny(x=12,y=40.5,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=21,y=40.5,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=30,y=40.5,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=39,y=40.5,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=12.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=12.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=16.7,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=15.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=18.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=21.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=24.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=20.9,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=25.1,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=28.1,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=27.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=30.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=33.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=32.3,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=36.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[9,-0.5,-0.5,0,0,0.7,0.4,1.4,0.6,2.1,0.7,2.8,0.6,3.5,0.4,4.2,0,4.7,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=36.5,y=40.5,warstwa=7,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[9,-0.5,-0.5,0,0,0.5,0.20,1,0.33,1.5,0.4,2,0.33,2.5,0.20,3,0,3.5,-0.5],szerokość_tekstury=32)
podstawy.obiekt(x=12.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
podstawy.obiekt(x=19.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
podstawy.obiekt(x=21.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
podstawy.obiekt(x=28.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
podstawy.obiekt(x=30.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
podstawy.obiekt(x=37.25,y=41,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
grafiki.interaktywna_świetlówka(x=15,y=40.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=19.5,y=40.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=24,y=40.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=28.5,y=40.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=33,y=40.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
podstawy.platforma(x=8.5,y=44,tekstura="rivets,0",wierzchołki=["v",4,-2,0,0.75,0,0.75,0.25,-2,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=9,y=44.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=9.5,y=45,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=10,y=45.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=10.5,y=46,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=11,y=46.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=8.5,y=44,tekstura="panels,1",wierzchołki=["v",6,0,0,3,3,2.85,3,0,0.15,-1,0.2,-1,0],jednostronna=True,R=128,G=128,B=128,warstwa=7)
podstawy.ściana(x=7.5,y=43,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,0,0,256,0,1,0,256,256,1,1,0,256,0,1,0,0])
podstawy.ściana(x=7.5,y=43,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,1,0,256,256,4,3,256,1024,4,4,0,1024,1,1,0,256])
mechanizmy.dźwignia(x=7.5,y=43,zdarzenia=["TechnicalHash1Open","TechnicalHash1Close"],zdarzenia_wewnętrzne=["L41","L42","L43","L44"],pozycja_początkowa=1)
#budynek generatora - parter - stróżówka
struktury.element_podłużny(x=42.5,y=30.5,warstwa=7,tekstura="TEXVent_pipe",grubość=0.8,trasa=[10,0,-2,0,8.5,0.5,9,3,9,3.5,9.5,3.5,10,3.5,10.5,4,11.5,5,12,5.5,12])
podstawy.platforma(x=42.5,y=43,warstwa=8,wierzchołki=["v",4,0,0,0,1,1.5,1,1.5,0],tekstura="bricks,0",R=128,G=128,B=128,powierzchnia="posadzka_hala")
podstawy.platforma(x=44,y=43,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=44.5,y=43.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=45,y=44,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=45.5,y=44.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=46,y=45,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=46.5,y=45.5,tekstura="rivets,0",wierzchołki=["v",4,0,0,0.75,0,0.75,0.25,0,0.25],jednostronna=True,R=192,G=192,B=192,warstwa=7)
podstawy.platforma(x=44,y=43,tekstura="panels,1",wierzchołki=["v",4,0,0,3,3,2.85,3,0,0.15],jednostronna=True,R=128,G=128,B=128,warstwa=7)
podstawy.ściana(x=42.5,y=42,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,0,0,256,0,1.5,0,256,384,1.5,1,0,384,0,1,0,0])
podstawy.ściana(x=42.5,y=42,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,1.5,0,256,384,4.5,3,256,1152,4.5,4,0,1152,1.5,1,0,384])
mechanizmy.dźwignia(x=42.5,y=41.75,zdarzenia=["PowerPlantOff","PowerPlantOn"],zdarzenia_wewnętrzne=["L11","L12","L13","L14"],pozycja_początkowa=1)
mechanizmy.dźwignia(x=48,y=44.5,zdarzenia=["LightOff","LightOn"],zdarzenia_wewnętrzne=["L21","L22","L23","L24"],obrót=-1)
#budynek generatora - pierwszy poziom techniczny
podstawy.platforma(x=7.5,y=39.5,wierzchołki=["v",4,0,0,0,1,1,1,1,0],tekstura="panels,2",powierzchnia="metal_rura",warstwa=8,R=128,G=128,B=128)
podstawy.platforma(x=11.5,y=39.5,wierzchołki=["v",4,0,0,0,1,30,1,30,0],tekstura="panels,1",powierzchnia="metal_rura",warstwa=8,R=128,G=128,B=128)
s = podstawy.platforma(x=8,y=39.7,wierzchołki=["v",4,0,0,0,0.6,4,0.6,4,0],tekstura="panels,0",powierzchnia="metal_rura",warstwa=7,zwróć_tekst=True)
mechanizmy.przesuwalny_element(s,["TechnicalHash1Close","TechnicalHash1Open"],["TechnicalHash1Closed","TechnicalHash1Opened"],10,1,[3,0])
struktury.element_podłużny(x=8,y=36,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=12,y=36,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=21,y=36,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=30,y=36,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=39,y=36,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=14.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=18.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=23.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=27.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=32.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=36.5,y=36,warstwa=5,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,3.5])
struktury.element_podłużny(x=12.5,y=36,warstwa=5,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[2,0,0,0,3.5],szerokość_tekstury=32)
struktury.element_podłużny(x=39.5,y=36,warstwa=6,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[2,0,0,0,3.5],szerokość_tekstury=32)
struktury.element_podłużny(x=40.7,y=36,warstwa=5,tekstura="TEXElectricity_cable_yellow",grubość=0.1,trasa=[2,0,0,0,3.5],szerokość_tekstury=32)
struktury.element_podłużny(x=7.5,y=36.5,warstwa=9,tekstura="TEXHydro_pipe_rusted",grubość=1.1,trasa=[2,0,0,41.5,0],kolizja=True,jednostronność=True)
struktury.element_podłużny(x=42,y=29,warstwa=9,tekstura="TEXHydro_pipe_rusted",grubość=1.1,trasa=[7,7,0,3,0,1.5,0.5,1,1,0.5,1.5,0,3,0,7],kolizja=True,jednostronność=True)
struktury.element_podłużny(x=9.5,y=32.2,warstwa=7,tekstura="TEXVent_pipe",grubość=0.8,trasa=[7,2,-4.5,2,-1,0,1,0,2.5,1,3.5,32,3.5,32.5,3],kolizja=True)
struktury.element_podłużny(x=7.5,y=38.25,warstwa=4,tekstura="TEXHydro_pipe",grubość=1.5,trasa=[2,0,0,41.5,0],kolor=[192,192,192])
struktury.element_podłużny(x=7.5,y=37.25,warstwa=5,tekstura="TEXSteel_stick",grubość=0.8,trasa=[2,0,0,41.5,0])
struktury.element_podłużny(x=7.5,y=39.25,warstwa=5,tekstura="TEXSteel_stick",grubość=0.8,trasa=[2,0,0,41.5,0])
struktury.element_podłużny(x=7.5,y=37.55,warstwa=5,tekstura="TEXElectricity_cable_green",grubość=0.08,trasa=[2,0,0,41.5,0],szerokość_tekstury=32)
struktury.element_podłużny(x=7.5,y=37.65,warstwa=5,tekstura="TEXElectricity_cable_red",grubość=0.06,trasa=[2,0,0,41.5,0],szerokość_tekstury=32)
struktury.element_podłużny(x=43,y=28.5,warstwa=4,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[2,0,0,0,7.5],kolizja=True)
struktury.element_podłużny(x=46.5,y=32,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,2.5,0],kolizja=True,jednostronność=True)
struktury.element_podłużny(x=46.5,y=28.5,warstwa=8,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[2,0,0,0,11])
struktury.schody(x=49,y=39.5,warstwa=5,wysokość=3.5,długość=3.5,stopnie=8,tekstura="bricks,0",obrót=-1)
grafiki.interaktywna_świetlówka(x=15,y=37.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=24,y=37.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
grafiki.interaktywna_świetlówka(x=33,y=37.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
podstawy.dźwięk(x=10,y=38.25,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=10)
podstawy.dźwięk(x=22,y=38.25,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=10)
podstawy.dźwięk(x=34,y=38.25,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=10)
podstawy.dźwięk(x=46,y=38.25,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=10)
początek_emisji = podstawy.animowany_obiekt(x=-0.5,y=-0.4,wysokość=1,tekstura="AOBSteam_cloud_concentrated",warstwa=7,zwróć_tekst=True)
koniec_emisji = podstawy.animowany_obiekt(x=-1.25,y=-1.5,wysokość=2.5,tekstura="AOBSteam_cloud_concentrated",warstwa=7,zwróć_tekst=True)
emitery = ""
emitery += pułapki.emiter(x=13,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=20,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=22,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=29,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=31,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=38,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery += pułapki.emiter(x=40,y=37,warstwa=7,długość=2.5,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery = operacje.element_potomny(emitery,"ruch",["linear",3,0,0,120,0,0,60,-1,-1,-1],True)
operacje.znikanie_zdarzeniowe(emitery,[2,"default","PowerPlantOff"],[1,"PowerPlantOn"])
koniec_emisji = ""
koniec_emisji += podstawy.animowany_obiekt(x=-1,y=-1.1,wysokość=2.5,tekstura="AOBSteam_cloud_concentrated",warstwa=7,czas_klatki=1,zwróć_tekst=True)
koniec_emisji += podstawy.animowany_obiekt(x=-0.875,y=-0.975,wysokość=2.25,tekstura="AOBSteam_cloud_concentrated",warstwa=7,czas_klatki=0.7,zwróć_tekst=True)
koniec_emisji += podstawy.animowany_obiekt(x=-0.75,y=-0.85,wysokość=2,tekstura="AOBSteam_cloud_concentrated",warstwa=7,czas_klatki=0.4,zwróć_tekst=True)
koniec_emisji += podstawy.strefa_obrażeń(x=0.25,y=0.15,wierzchołki=obiekty.wielokąt_foremny(n=6,bok=1,początek=0),obrażenia=["d",2,15,6,0,0],zwróć_tekst=True)
koniec_emisji = operacje.element_potomny(koniec_emisji,"ruch",ruchy.wstrząsy(0.1,7,0,10),True)
emitery = pułapki.emiter(x=42.5,y=31.5,warstwa=7,długość=2.5,zakończenie=1,kąt_emisji=25,obrót=30,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
operacje.znikanie_zdarzeniowe(emitery,[2,"default","PowerPlantOff"],[1,"PowerPlantOn"])
podstawy.obiekt(x=43.75,y=30.25,wysokość=2,tekstura="OBJGrated_vent",warstwa=4)
#budynek generatora - drugi poziom techniczny
podstawy.platforma(x=41.5,y=28,tekstura="panels,0",wierzchołki=["v",4,0,0,0,0.5,5.5,0.5,5.5,0],R=192,B=192,G=192,warstwa=7)
struktury.element_podłużny(x=42,y=28.5,warstwa=6,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,7],kolor=[192,192,192],kolizja=True)
struktury.element_podłużny(x=41,y=21.5,warstwa=9,tekstura="TEXHydro_pipe_rusted",grubość=1.1,trasa=[5,0,0.5,0,2,0.5,3,1.5,3.5,8,3.5],kolizja=True)
struktury.element_podłużny(x=43,y=24.5,warstwa=8,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[2,0,0,0,4])
struktury.element_podłużny(x=46.5,y=25.5,warstwa=7,tekstura="TEXElectricity_cable_yellow",grubość=0.07,trasa=[9,-0.5,-0.5,0,0,0.4,0.20,0.8,0.33,1.2,0.4,1.6,0.33,2,0.20,2.4,0,3.5,-0.5],szerokość_tekstury=32)
struktury.element_podłużny(x=43.5,y=25.5,warstwa=8,tekstura="TEXElectricity_cable_blue",grubość=0.1,trasa=[3,0,0,-0.6,0.5,-0.6,3],szerokość_tekstury=64)
struktury.element_podłużny(x=43.5,y=25.5,warstwa=6,tekstura="TEXElectricity_cable_blue",grubość=0.1,trasa=[2,-0.6,3,-0.6,4],szerokość_tekstury=64)
struktury.element_podłużny(x=42.5,y=24.5,warstwa=8,tekstura="TEXVent_pipe",grubość=0.8,trasa=[2,0,0,0,4])
grafiki.interaktywna_świetlówka(x=43.5,y=25.5,warstwa=4,długość=3,włącznik="LightOn",wyłącznik="LightOff")
początek_emisji = podstawy.animowany_obiekt(x=-0.5,y=-0.4,wysokość=1,tekstura="AOBSteam_cloud_concentrated",warstwa=7,zwróć_tekst=True)
koniec_emisji = ""
koniec_emisji += podstawy.animowany_obiekt(x=-1.25,y=-1,wysokość=2.5,tekstura="AOBSteam_cloud_concentrated",warstwa=7,zwróć_tekst=True)
koniec_emisji += podstawy.animowany_obiekt(x=-1.125,y=-0.875,wysokość=2.25,tekstura="AOBSteam_cloud_concentrated",warstwa=7,czas_klatki=0.7,zwróć_tekst=True)
koniec_emisji += podstawy.animowany_obiekt(x=-1,y=-0.75,wysokość=2,tekstura="AOBSteam_cloud_concentrated",warstwa=7,czas_klatki=0.4,zwróć_tekst=True)
emitery = ""
emitery += pułapki.emiter(x=48,y=25.5,warstwa=7,długość=5,zakończenie=0.5,kąt_emisji=15,obrót=90,początek_emisji=początek_emisji,koniec_emisji=koniec_emisji,zwróć_tekst=True)
emitery = operacje.element_potomny(emitery,"ruch",["linear",3,0,0,100,0,0,100,-1,-1,-1],True)
operacje.znikanie_zdarzeniowe(emitery,[2,"default","PowerPlantOff"],[1,"PowerPlantOn"])
struktury.element_podłużny(x=11.875,y=28,warstwa=6,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,7.3],kolor=[192,192,192])
struktury.schody(x=7.5,y=28,wysokość=1,długość=6,stopnie=2,przesunięcie=5,tekstura="bricks,0",kolor=[128,128,128],warstwa=8)
struktury.element_podłużny(x=12,y=30.5,warstwa=5,tekstura="TEXSteel_stick",grubość=0.8,trasa=[2,0,0,30,0],kolor=[128,128,128])
struktury.element_podłużny(x=12,y=31.1,warstwa=5,tekstura="TEXSteel_stick",grubość=0.8,trasa=[2,0,0,30,0],kolor=[128,128,128])
podstawy.ściana(x=12,y=31.5,wierzchołki=["v",4,0,0,0,4,30,4,30,0],tekstura="concrete,0",warstwa=5,R=128,G=128,B=128)
podstawy.ściana(x=25,y=31.5,warstwa=5,tekstura="panels,0",wierzchołki=["v",4,0,0,0,4,4,4,4,0],R=64,G=64,B=64)
podstawy.obiekt(x=25,y=31.5,wysokość=4,tekstura="OBJSteel_technical_gate",warstwa=5,R=192,G=192,B=192)
podstawy.ściana(x=14,y=31.5,warstwa=5,tekstura="panels,1",wierzchołki=["v",4,0,0,0,4,4,4,4,0],R=64,G=64,B=64)
podstawy.obiekt(x=14,y=31.5,wysokość=4,tekstura="OBJSteel_technical_gate",warstwa=5,R=192,G=192,B=192)
podstawy.ściana(x=36,y=31.5,warstwa=5,tekstura="panels,2",wierzchołki=["v",4,0,0,0,4,4,4,4,0],R=64,G=64,B=64)
podstawy.obiekt(x=36,y=31.5,wysokość=4,tekstura="OBJSteel_technical_gate",warstwa=5,R=192,G=192,B=192)
podstawy.platforma(x=10.5,y=22,wierzchołki=["v",4,0,0,3,0,2,1,0,1],warstwa=8,R=128,G=128,B=128,tekstura="panels,1")
podstawy.platforma(x=7.5,y=22,wierzchołki=["v",4,0,0,1,0,1,1,0,1],warstwa=8,R=128,G=128,B=128,tekstura="panels,2")
struktury.element_podłużny(x=11.875,y=23,warstwa=6,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,4],kolor=[192,192,192])
mechanizmy.dźwignia(x=7.5,y=25,zdarzenia=["TechnicalHash2Open","TechnicalHash2Close"],zdarzenia_wewnętrzne=["L31","L32","L33","L34"],pozycja_początkowa=1)
s = podstawy.platforma(x=8,y=22.2,wierzchołki=["v",4,0,0,3,0,3,0.6,0,0.6],warstwa=7,tekstura="panels,0",zwróć_tekst=True)
mechanizmy.przesuwalny_element(s,["TechnicalHash2Close","TechnicalHash2Open"],["TechnicalHash2Closed","TechnicalHash2Opened"],10,1,[2,0])
#budynek generatora - drugi poziom techniczny - wieże_instalacyjne
podstawy.platforma(x=29.5,y=22,tekstura="concrete,0",wierzchołki=["v",4,0,0,6,0,5,1,1,1],R=196,B=196,G=196,warstwa=7)
podstawy.platforma(x=30.5,y=27.5,tekstura="panels,0",wierzchołki=["v",4,0,0,0,1.5,4,1.5,4,0],R=196,B=196,G=196,warstwa=7)
podstawy.platforma(x=30.5,y=23,tekstura="panels,2",wierzchołki=["v",4,0,0,0,0.5,4,0.5,4,0],R=196,B=196,G=196,warstwa=7)
podstawy.ściana(x=31,y=23.5,tekstura="panels,1",wierzchołki=["v",4,0,0,0,4,3,4,3,0],R=128,B=128,G=128,warstwa=5)
struktury.element_podłużny(x=31.5,y=29,warstwa=5,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=33.5,y=29,warstwa=5,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=32.5,y=29,warstwa=5,tekstura="TEXHydro_pipe_rusted",grubość=1.1,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=31.7,y=29,warstwa=5,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[4,0,-5.5,0,3.5,-1,4.5,-1,6.5],kolor=[192,192,192])
struktury.element_podłużny(x=33.3,y=29,warstwa=5,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[4,0,-5.5,0,3.5,1,4.5,1,6.5],kolor=[192,192,192])
podstawy.obiekt(x=31,y=27.75,wysokość=1,tekstura="OBJGrated_vent",warstwa=9)
podstawy.obiekt(x=33,y=27.75,wysokość=1,tekstura="OBJGrated_vent",warstwa=9)
grafiki.interaktywna_świetlówka(x=31,y=23.5,warstwa=7,długość=3,włącznik="LightOn",wyłącznik="LightOff")
podstawy.ściana(x=30.5,y=26.5,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,0,0,256,0,4,0,256,1024,4,1,0,1024,0,1,0,0])
podstawy.dźwięk(x=32.5,y=32,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=15)
podstawy.platforma(x=18.5,y=22,tekstura="concrete,0",wierzchołki=["v",4,0,0,6,0,5,1,1,1],R=196,B=196,G=196,warstwa=7)
podstawy.platforma(x=19.5,y=27.5,tekstura="panels,0",wierzchołki=["v",4,0,0,0,1.5,4,1.5,4,0],R=196,B=196,G=196,warstwa=7)
podstawy.platforma(x=19.5,y=23,tekstura="panels,2",wierzchołki=["v",4,0,0,0,0.5,4,0.5,4,0],R=196,B=196,G=196,warstwa=7)
podstawy.ściana(x=20,y=23.5,tekstura="panels,1",wierzchołki=["v",4,0,0,0,4,3,4,3,0],R=128,B=128,G=128,warstwa=5)
struktury.element_podłużny(x=20.5,y=29,warstwa=5,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=22.5,y=29,warstwa=5,tekstura="TEXSteel_stick",grubość=0.75,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=21.5,y=29,warstwa=5,tekstura="TEXHydro_pipe_rusted",grubość=1.1,trasa=[2,0,0,0,6.5])
struktury.element_podłużny(x=20.7,y=29,warstwa=5,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[4,0,-5.5,0,3.5,-1,4.5,-1,6.5],kolor=[192,192,192])
struktury.element_podłużny(x=22.3,y=29,warstwa=5,tekstura="TEXHydro_pipe",grubość=0.8,trasa=[4,0,-5.5,0,3.5,1,4.5,1,6.5],kolor=[192,192,192])
podstawy.obiekt(x=20,y=27.75,wysokość=1,tekstura="OBJGrated_vent",warstwa=9)
podstawy.obiekt(x=22,y=27.75,wysokość=1,tekstura="OBJGrated_vent",warstwa=9)
grafiki.interaktywna_świetlówka(x=20,y=23.5,warstwa=7,długość=3,włącznik="LightOn",wyłącznik="LightOff")
podstawy.ściana(x=19.5,y=26.5,warstwa=8,tekstura="TEXSteel_guardrail",wierzchołki=["vt",4,0,0,256,0,4,0,256,1024,4,1,0,1024,0,1,0,0])
podstawy.dźwięk(x=21.5,y=32,dźwięk="RURY",głośność=60,wyciszanie=0.5,zasięg=15)
mosty = podstawy.platforma(x=23,y=28,wierzchołki=["v",4,0,0,4,0,4,0.5,0,0.5],tekstura="panels,1",warstwa=6,zwróć_tekst=True)
mosty += podstawy.platforma(x=34,y=28,wierzchołki=["v",4,0,0,4,0,4,0.5,0,0.5],tekstura="panels,1",warstwa=6,zwróć_tekst=True)
mechanizmy.przesuwalny_element(mosty,zdarzenia=["PowerPlantOn","PowerPlantOff"],zdarzenia_wewnętrzne=["TechnicalBridgeClose","TechnicalBridgeOpen"],czas_przesunięcia=20,przesunięcie=[-3.5,0])
mosty = podstawy.platforma(x=16,y=28,wierzchołki=["v",4,0,0,4,0,4,0.5,0,0.5],tekstura="panels,0",warstwa=6,zwróć_tekst=True)
mosty += podstawy.platforma(x=27,y=28,wierzchołki=["v",4,0,0,4,0,4,0.5,0,0.5],tekstura="panels,0",warstwa=6,zwróć_tekst=True)
mosty += podstawy.platforma(x=38,y=28,wierzchołki=["v",4,0,0,4,0,4,0.5,0,0.5],tekstura="panels,0",warstwa=6,zwróć_tekst=True)
mechanizmy.przesuwalny_element(mosty,zdarzenia=["PowerPlantOn","PowerPlantOff"],zdarzenia_wewnętrzne=["TechnicalBridgeClose","TechnicalBridgeOpen"],czas_przesunięcia=20,przesunięcie=[3.5,0])
mosty = podstawy.platforma(x=11.75,y=28.25,wierzchołki=["v",4,-0.25,-0.25,4.25,-0.25,4.25,0.25,-0.25,0.25],tekstura="panels,2",warstwa=6,zwróć_tekst=True)
mechanizmy.obracalny_element(mosty,zdarzenia=["PowerPlantOn","PowerPlantOff"],zdarzenia_wewnętrzne=["TechnicalBridgeClose","TechnicalBridgeOpen"],czas_obrotu=20,obrót=90)
#budynek generatora - przejście rurami zewnętrznymi
podstawy.strefa_zdarzeń(x=6.5,y=29,wierzchołki=["v",4,0,0,0,2,1,2,1,0],zdarzenia=["e",1,"OutPipeEntered"])
podstawy.strefa_zdarzeń(x=8.5,y=28.5,wierzchołki=["v",4,0,0,0,3,1,3,1,0],zdarzenia=["e",1,"OutPipeLeft"])
podstawy.strefa_zdarzeń(x=6.5,y=10,wierzchołki=["v",4,0,0,0,2,1,2,1,0],zdarzenia=["e",1,"OutPipeEntered"])
podstawy.strefa_zdarzeń(x=8.5,y=9.5,wierzchołki=["v",4,0,0,0,3,1,3,1,0],zdarzenia=["e",1,"OutPipeLeft"])
podstawy.ściana(x=6.5,y=9.5,tekstura="bricks,0",wierzchołki=["v",4,0,0,0,3,1,3,1,0],R=128,B=128,G=128,warstwa=8)
podstawy.ściana(x=6.5,y=28.5,tekstura="bricks,0",wierzchołki=["v",4,0,0,0,3,1,3,1,0],R=128,B=128,G=128,warstwa=8)
s = struktury.element_podłużny(x=4.5,y=11,warstwa=7,tekstura="TEXSteel_stick",grubość=3,trasa=[6,4,0,1.5,0,0,1.5,0,17.5,1.5,19,4,19],zwróć_tekst=True)
s += struktury.element_podłużny(x=6,y=15.5,warstwa=7,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],zwróć_tekst=True)
s += struktury.element_podłużny(x=6,y=20,warstwa=7,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],zwróć_tekst=True)
s += struktury.element_podłużny(x=6,y=24.5,warstwa=7,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],zwróć_tekst=True)
operacje.znikanie_zdarzeniowe(s,[1,"OutPipeEntered"],[2,"default","OutPipeLeft"])
struktury.element_podłużny(x=4.5,y=11,warstwa=4,tekstura="TEXSteel_stick",grubość=3,trasa=[6,4,0,1.5,0,0,1.5,0,17.5,1.5,19,4,19],kolor=[128,128,128])
struktury.element_podłużny(x=6,y=15.5,warstwa=4,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],kolor=[128,128,128])
struktury.element_podłużny(x=6,y=20,warstwa=4,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],kolor=[128,128,128])
struktury.element_podłużny(x=6,y=24.5,warstwa=4,tekstura="TEXSteel_stick",grubość=3,trasa=[2,0,0,0.5,0],kolor=[128,128,128])
struktury.element_podłużny(x=8.5,y=9.7,warstwa=4,tekstura="TEXSteel_stick",grubość=0.4,trasa=[6,0,0,-3,0,-5.3,2.3,-5.3,19.3,-3,21.6,0,21.6],kolizja=True,kolor=[192,192,192])
struktury.element_podłużny(x=8.5,y=12.3,warstwa=4,tekstura="TEXSteel_stick",grubość=0.4,trasa=[5,0,0,-2,0,-2.7,0.7,-2.7,1.9,-2,1.9],kolizja=True,kolor=[192,192,192])
struktury.element_podłużny(x=6.5,y=16.8,warstwa=4,tekstura="TEXSteel_stick",grubość=0.4,trasa=[4,0,0,-0.7,0,-0.7,1.9,0,1.9],kolizja=True,kolor=[192,192,192])
struktury.element_podłużny(x=6.5,y=21.3,warstwa=4,tekstura="TEXSteel_stick",grubość=0.4,trasa=[4,0,0,-0.7,0,-0.7,1.9,0,1.9],kolizja=True,kolor=[192,192,192])
struktury.element_podłużny(x=6.5,y=25.8,warstwa=4,tekstura="TEXSteel_stick",grubość=0.4,trasa=[5,0,0,-0.7,0,-0.7,2.2,0,2.9,2,2.9],kolizja=True,kolor=[192,192,192])
podstawy.obiekt(x=9,y=28.5,wysokość=1.5,tekstura="OBJGrated_vent",warstwa=4)
struktury.element_podłużny(x=7,y=31.5,warstwa=7,tekstura="TEXSteel_stick_rusted",grubość=0.35,trasa=[3,0,0,1,1,1,4.5])

lvl.save('Elektrownia_Weglowa_X3Y2.xml')