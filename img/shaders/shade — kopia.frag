...
{
	//float intensity = 0.2;
	//if(texture2D(texture, gl_TexCoord[0].xy) == vec4(1.0,1.0,1.0,1.0))
	//{
	//	intensity = max(1.0 - distance(gl_TexCoord[0].xy, light_pos), intensity);
	//}
	gl_FragColor = vec4(distance(gl_TexCoord[0].xy, light_pos));
	//if(distance(gl_TexCoord[0].xy, light_pos) > 0.5)
